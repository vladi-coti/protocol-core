...
library ForceActionsFacetImpl {
	...
	function forceClosePosition(
		...
		// Decrypt requestedClosePrice for calculations
		...
		if (quote.positionType == PositionType.LONG) {
			require(
				sig.highest >= requestedClosePrice + (requestedClosePrice * symbolLayout.forceCloseGapRatio[quote.symbolId]) / 1e18,
				"PartyAFacet: Requested close price not reached"
			);
			closePrice = requestedClosePrice + (requestedClosePrice * maLayout.forceClosePricePenalty) / 1e18;
			closePrice = closePrice > sig.averagePrice ? closePrice : sig.averagePrice; // max
		} else {
			require(
				sig.lowest <= requestedClosePrice - (requestedClosePrice * symbolLayout.forceCloseGapRatio[quote.symbolId]) / 1e18,
				"PartyAFacet: Requested close price not reached"
			);
			closePrice = requestedClosePrice - (requestedClosePrice * maLayout.forceClosePricePenalty) / 1e18;
		...
		quoteIds[0] = quoteId;
		filledAmounts[0] = quantityToClose;
