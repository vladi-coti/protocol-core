...
library PrivateAccountFacetImpl {
	...
	function deposit(address user, uint256 amount) internal {
		...
		PrivateAccountStorage.layout().balances[user] += amountWith18Decimals;
	}

	function withdraw(address user, uint256 amount) internal {
		PrivateAccountStorage.Layout storage accountLayout = PrivateAccountStorage.layout();
		GlobalAppStorage.Layout storage appLayout = GlobalAppStorage.layout();
		require(
	...
	function deallocate(uint256 amount, SingleUpnlSig memory upnlSig) internal {
		...
		gtBool sufficientBalance = gtAvailableBalance.ge(gtAmount);
		gtBool allChecks = availablePositive.and(sufficientBalance);

		require(MpcCore.decrypt(allChecks), "AccountFacet: Insufficient available balance or will be liquidatable");
	...
	function transferAllocation(uint256 amount, address origin, address recipient, SingleUpnlSig memory upnlSig) internal {
		...
		require(MpcCore.decrypt(allChecks), "PartyBFacet: Insufficient available balance or will be liquidatable");
