...
import {
	...
	PrivateQuoteParamsStruct,
	QuoteBasicParamsStruct,
...
async function getListFormatOfQuoteRequest(
	...
	user: User,
	selector: string,
): Promise<[QuoteBasicParamsStruct, PrivateQuoteParamsStruct, SingleUpnlAndPriceSigStruct]> {
	return user.buildQuoteCalldataArgs(request, selector)
}
...
async function getListFormatOfCloseRequest(
	...
	return user.buildCloseRequestCalldataArgs(request, selector)
}
...
async function getListFormatOfOpenRequest(
	...
	hedger: Hedger,
	selector: string,
): Promise<[PrivateOpenPositionParamsStruct, PairUpnlAndPriceSigStruct]> {
	const {encryptedParams, upnlSig} = await hedger.buildOpenPositionCalldataArgs(request, selector)
	return [encryptedParams, upnlSig]
}
...
async function getListFormatOfFillCloseRequest(
	...
	hedger: Hedger,
	selector: string,
): Promise<[PrivateClosePositionParamsStruct, PairUpnlAndPriceSigStruct]> {
	const {encryptedParams, upnlSig} = await hedger.buildFillCloseRequestCalldataArgs(request, selector)
	return [encryptedParams, upnlSig]
}
...
export function shouldBehaveLikeMultiAccount() {
	...
	let multiAccountCallSelector: string
	let symmioPartyBCallSelector: string
	...
	beforeEach(async () => {
		...
		await user2.setup()
		await user2.setBalances(decimal(10000n), decimal(6000n), decimal(6000n))

		hedger = new Hedger(context, context.signers.hedger)
		await hedger.setup()
		await hedger.setBalances(decimal(10000n), decimal(10000n))

		admin = new Hedger(context, context.signers.admin)
		await admin.setup()
		await admin.setBalances(decimal(10000n), decimal(10000n))
	...
	describe("Account Management", function () {
		...
		describe("delegatedAccesses", function () {
			...
			beforeEach(async () => {
				...
				selector = context.partyAFacet.interface.getFunction("sendQuote").selector
	...
	describe("send new quote", function () {
		...
		it("Should be able to send Quotes", async () => {
			let quoteRequest1 = limitQuoteRequestBuilder().partyBWhiteList([await symmioPartyB.getAddress()]).build()
			let sendQuote1 = context.partyAFacet.interface.encodeFunctionData("sendQuote", await getListFormatOfQuoteRequest(quoteRequest1, user, multiAccountCallSelector))
			...
			let decryptedRequestedPrice = await decryptUint256(context, quote.requestedOpenPrice, context.signers.user)
			let decryptedQuantity = await decryptUint256(context, quote.quantity, context.signers.user)
		...
		describe("Locking quotes", function () {
			beforeEach(async () => {
				const partyBAddress = await symmioPartyB.getAddress()
				let quoteRequest1 = marketQuoteRequestBuilder().partyBWhiteList([partyBAddress]).affiliate(await multiAccount.getAddress()).build()
				let sendQuote1 = context.partyAFacet.interface.encodeFunctionData("sendQuote", await getListFormatOfQuoteRequest(quoteRequest1, user, multiAccountCallSelector))
				let quoteRequest2 = marketQuoteRequestBuilder().partyBWhiteList([partyBAddress]).affiliate(await multiAccount.getAddress()).positionType(PositionType.SHORT).build()
				let sendQuote2 = context.partyAFacet.interface.encodeFunctionData("sendQuote", await getListFormatOfQuoteRequest(quoteRequest2, user, multiAccountCallSelector))

				await context.collateral.connect(context.signers.admin).mint(partyBAddress, decimal(1000000n))
			...
			describe("Open quotes", function () {
				...
				it("Should be able to Open Quote", async () => {
					...
					let openPositionParams = await getListFormatOfOpenRequest(openPosition2, admin, symmioPartyBCallSelector)
				...
				describe("Request to close", function () {
					...
					it("request to close position", async () => {
						...
						let closeRequestParams = await getListFormatOfCloseRequest(closeRequest1, user, multiAccountCallSelector)
					...
					describe("Request to fill close", function () {
						beforeEach(async () => {
							...
							let closeRequestParams = await getListFormatOfCloseRequest(closeRequest, user, multiAccountCallSelector)
