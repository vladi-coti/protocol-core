...
/**
 * H-26: force-close insolvency path passes post-close simulated available (locks
 * for the closing quote already unlocked in that number) into
 * liquidatePartyBFromAvailable while quote storage / partyBLockedBalances.lf
 * still include that quote.
 *
 * Invariant: implied remainingLf must be <= LF_total - lf_closed_quote.
 * Bug: remainingLf still counts closed-quote LF → violates invariant.
 *
 * PartyA (force-close caller) receives alloc - remainingLf*(1 - liquidatorShare).
 */
export function shouldBehaveLikeAuditH26(): void {
	describe("force-close liquidation post-close available vs open quote LF", function () {
		it("H-26: remainingLf must not include LF already unlocked in post-close available", async function () {
			...
			const marketPrice = decimal(35n, 17)
			await user.forceClosePosition(
				short.quoteId,
				await getDummyHighLowPriceSig(startTime, endTime, 0n, decimal(10n), marketPrice, marketPrice, 0n, 0n, 0n),
			)

			const partyAGain = (await user.getBalanceInfo()).allocatedBalances - partyABefore
			const liquidated = await context.viewFacet.isPartyBLiquidated(await hedger.getAddress(), partyA)
			...
			// gain = alloc - remainingLf * (1 - share)  ⇒  remainingLf = (alloc - gain) / (1 - share)
			const retained = hedgerBefore.allocatedBalances - partyAGain
			expect(retained).to.be.gt(0n, "need remainingLf > 0 to observe LF mismatch")
			const remainingLf = (retained * one) / (one - liquidatorShare)
			const lfAfterUnlock = hedgerBefore.lockedLf - shortLf

			expect(remainingLf).to.be.lte(
				lfAfterUnlock,
				`H-26: remainingLf must exclude closed-quote LF already unlocked in available; ` +
					`remainingLf=${remainingLf} lfAfterUnlock=${lfAfterUnlock} shortLf=${shortLf} ` +
					`totalLf=${hedgerBefore.lockedLf} gain=${partyAGain} alloc=${hedgerBefore.allocatedBalances}`,
			)
		})
	})
}
